...
그다음 볼륨을 센다 — **값이지 판정이 아니다.**

```
node "${CLAUDE_PLUGIN_ROOT}/specprobe.mjs" SPEC.md
```

- 출력의 `volume`에서 **활성 문장 수 · [추론] 비율 · 표 행수 · 아카이브 후보 수** 네 값만 한두 줄로 보고한다.
- **임계를 네가 판정하지 마라.** «많다 / 접자»는 사용자가 정한다. 아카이브는 후보 제시까지고,
  승인 없이 옮기지 않는다. `선택 대기` 행은 후보에 올리지 않는다.
