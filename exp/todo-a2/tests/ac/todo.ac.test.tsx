...
describe("Todo AC (frozen)", () => {
  ...
  it("AC-02 reject blank: whitespace-only input adds nothing (valid input still works)", async () => {
    const user = setup();
    await add(user, "할일A");
    expect(items()).toHaveLength(1);
    await add(user, "   ");
  ...
  it("AC-03 toggle: clicking the checkbox marks done, clicking again undoes it", async () => {
    const user = setup();
    await add(user, "할일A");
  ...
  it("AC-04 delete: removing the 2nd of 3 leaves the others in order", async () => {
    const user = setup();
  ...
  it("AC-05 remaining count: todo-count tracks the number of not-done items", async () => {
    const user = setup();
    for (const s of ["A", "B", "C"]) await add(user, s);
  ...
  it("AC-06 filters: all / active / completed show the right subset", async () => {
    const user = setup();
    for (const s of ["A", "B", "C"]) await add(user, s);
    await user.click(toggleOf(0));
  ...
  it("AC-07 persistence: items and done state survive a remount, via localStorage", async () => {
    const user = setup();
    await add(user, "A");
    await add(user, "B");
    await user.click(toggleOf(0));
  ...
  it("AC-08 accessible names: input, toggle and delete each expose a non-empty name", async () => {
    const user = setup();
    await add(user, "할일A");
    const row = items()[0];
