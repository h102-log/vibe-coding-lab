...
// 에이전트가 자기 스크립트를 만들어 돌린다 ((d)#1은 `npm run test:dev`). 커맨드에 "vitest"가
// 없어도 자체 실행으로 센다. `test:ac`는 위 IS_ORACLE이 먼저 잡으므로 여기 걸리지 않는다.
const IS_VITEST = /vitest|npm\s+(run\s+)?test/;
// 파일명 규약에 기대지 않는다. (d)#1이 `tests/dev/todo.check.tsx`로 이름을 짓고
// "다른 러너의 기본 include에 걸리지 않는다"고 설정에 적었다 — 이름으로 세면 조용히 0이 된다.
// 쓴 내용에 vitest 단언과 케이스 선언이 둘 다 있으면 테스트 파일로 본다 (selftest.sh와 같은 규칙).
const IS_TEST_FILE = /\.(test|spec)\.[jt]sx?$/;
const looksLikeTest = (src) =>
  /\bexpect\s*\(/.test(src) && /(^|\s)(it|test)\s*\(/m.test(src);
const IS_SRC = /src[\\/]/;
...
for (const e of entries) {
  ...
  for (const b of content) {
    if (b.type === "tool_use") {
      const { command = "", file_path = "", content = "", new_string = "" } = b.input ?? {};
      ...
      if (
        WRITES.has(b.name) && !/tests[\\/]ac[\\/]/.test(file_path) &&
        (IS_TEST_FILE.test(file_path) || looksLikeTest(content || new_string))
      ) testFiles.set(file_path, false);
